...
export const MasterSettings: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'types' | 'roles'>('types');
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="glass-panel" style={{ padding: 'var(--spacing-6)', marginBottom: 'var(--spacing-8)' }}>
        <div style={{ display: 'flex', gap: 'var(--spacing-4)', marginBottom: 'var(--spacing-6)', borderBottom: '1px solid var(--border-color)', paddingBottom: 'var(--spacing-4)' }}>
          <button 
            className={`btn ${activeTab === 'types' ? 'primary' : 'outline'}`} 
            onClick={() => setActiveTab('types')}
          >
            職種・タイプ別評価
          </button>
          <button 
            className={`btn ${activeTab === 'roles' ? 'primary' : 'outline'}`} 
